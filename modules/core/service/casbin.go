...
// @param:       authorityId string, casbinInfos []requests.CasbinInfo
// @return:      error

type CasbinService struct {
}

func (casbinService *CasbinService) UpdateCasbin(authorityId string, casbinInfos []requests.CasbinInfo) error {
	casbinService.ClearCasbin(0, authorityId)
	var rules [][]string
	for _, v := range casbinInfos {
		cm := entities.CasbinModel{
			Ptype:       "p",
...
func (casbinService *CasbinService) UpdateCasbinApi(oldPath string, newPath string, oldMethod string, newMethod string) error {
	err := global.DB.Table("casbin_rule").Model(&entities.CasbinModel{}).Where("v1 = ? AND v2 = ?", oldPath, oldMethod).Updates(map[string]interface{}{
		"v1": newPath,
		"v2": newMethod,
	}).Error
	return err
...
// @return:      pathMaps []requests.CasbinInfo

func (casbinService *CasbinService) GetPolicyPathByAuthorityId(authorityId string) (pathMaps []requests.CasbinInfo) {
	e := casbinService.Casbin()
	list := e.GetFilteredPolicy(0, authorityId)
	for _, v := range list {
		pathMaps = append(pathMaps, requests.CasbinInfo{
			Path:   v[1],
			Method: v[2],
		})
	}
